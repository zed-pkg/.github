...
Safety rules:
- Zed registry requirements move only when the dependency package's declared \`v<version>\` tag resolves to that dependency repository's current default-branch tip, and never downgrade a simple semantic-version requirement.
