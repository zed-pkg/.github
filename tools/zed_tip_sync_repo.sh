...
for package in metadata.get("packages", []):
    ...
    raw = source[4:]
    split = urlsplit(raw)
    path = split.path[:-4] if split.path.endswith(".git") else split.path
    base = urlunsplit((split.scheme.lower(), split.netloc.lower(), path.rstrip("/"), "", "")).lower()
    by_repo.setdefault(base, set()).add(source)

bad = {repo: sorted(sources) for repo, sources in by_repo.items() if len(sources) > 1}
if bad:
    for repo, sources in sorted(bad.items()):
        print(f"multiple Git source identities for {repo}:", file=sys.stderr)
...
if [[ -f Cargo.toml ]]; then
  ...
  if ! cmp -s Cargo.toml "$work/Cargo.toml.before"; then
    cargo_ok=true
    if ! timeout 300s cargo metadata --format-version 1 >"$work/cargo-metadata.json" 2>"$work/cargo-metadata.err"; then
      warn 'full cargo metadata resolution failed; reverting Cargo dependency-tip changes'
      preserve_diag cargo-metadata.err "$work/cargo-metadata.err"
      cargo_ok=false
    elif ! cargo_sources_are_coherent "$work/cargo-metadata.json" >"$work/cargo-source-coherence.out" 2>"$work/cargo-source-coherence.err"; then
      warn 'resolved Cargo graph contains multiple revisions of one Git repository; reverting Cargo dependency-tip changes'
      preserve_diag cargo-source-coherence.err "$work/cargo-source-coherence.err"
      cargo_ok=false
    elif $had_cargo_lock && ! timeout 300s cargo metadata --format-version 1 --locked >"$work/cargo-metadata-locked.json" 2>"$work/cargo-metadata-locked.err"; then
      warn 'regenerated Cargo.lock is not reproducible under --locked; reverting Cargo dependency-tip changes'
      preserve_diag cargo-metadata-locked.err "$work/cargo-metadata-locked.err"
      cargo_ok=false
    fi
    if ! $cargo_ok; then
      cp "$work/Cargo.toml.before" Cargo.toml
      if $had_cargo_lock; then cp "$work/Cargo.lock.before" Cargo.lock; else rm -f Cargo.lock; fi
    elif ! $had_cargo_lock; then
      rm -f Cargo.lock
    fi
  fi
fi
...
- Exact Git \`rev\` dependencies move to current repository tips only when full Cargo resolution succeeds, the lockfile is regenerated when tracked, and the resolved graph does not contain multiple source identities for the same Git repository.
- \`.zpkg.lock\` is regenerated by current-tip \`zed\` and retained only when \`zed validate --require-lock --json\` succeeds.
- Tracked \`Cargo.lock\` files are regenerated by Cargo and rechecked with \`--locked\`; repositories that intentionally do not track a lockfile remain lockfile-free.
